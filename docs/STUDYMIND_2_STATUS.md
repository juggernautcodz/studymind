...
- **HIGH — general server-backed Topic content can still open empty local tabs.** Phase 8B performs an authenticated canonical server fetch only when entering Cards or Quiz from a Study Today recommendation because durable evidence requires server IDs. Ordinary Topic entry and fresh-device hydration still read notes, cards, and quizzes from device-local storage after hierarchy-only hydration. Resolving the broader gap requires a bounded server-to-cache reconciliation contract; Phase 8B did not expand into that separate remediation.
...
- `npm run test:study-evidence` — 10 passed; covers canonical IDs, durable flashcard success/failure ordering, authenticated owned-quiz service behavior, server scoring, Mastery update dispatch, stable-submission idempotency/conflict handling, client failure propagation, and relevant cache refresh keys
